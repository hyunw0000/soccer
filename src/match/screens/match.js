...
export default function matchScreen(root, ctx) {
  ...
  function updateBanners() {
    banner.classList.toggle("show", matchPhase === 'playing' && paused && sim.phase === "playing" && !concedeChoicePending);
    phaseBanner.classList.toggle("show", sim.phase === "fulltime" && !concedeChoicePending);
  ...
  function formatMatchClock() {
    const totalSeconds = Math.floor(sim.tick * PARAMS.dt * 60);
