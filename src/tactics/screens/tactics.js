...
export default function tacticsScreen(root, ctx) {
  ...
  const formationId = resolveFormationId(
    state.startingLineup?.formationId ?? state.formation,
  );
  ...
  const usable =
    saved &&
    saved.assignments.every(
      (a) => a.playerId && squadPlayerIds.includes(a.playerId),
    );
  const initial = setCaptain(
    usable
      ? saved
      : autoLineup(squadPlayerIds, formationId, findById, { captainId }),
    captainId,
  );
  ...
  function createHintBox() {
    ...
    const show = (field) => {
      ...
      text.textContent =
        field?.hint ?? "항목에 마우스를 올리면 설명이 나옵니다.";
    };
    show(null);
  ...
  const resetBtn = el("button", {
    ...
    onclick: () =>
      applyPreset(
        replaceActive(book, createPreset(book.selected[book.activeSet])),
      ),
  });
  ...
  const floatingKickoffBtn=el("button",{
    class:"primary tactics-floating-kickoff",
    type:"button",
    text:"역사를 바꿀 KICK OFF →",
    hidden:true,
    onclick:()=>kickoffBtn.click(),
  });

  const previousBtn=el("button",{
    class:"ghost tactics-match-card__back",
    type:"button",
    text:"← 이전",
    onclick:()=>{
      persist();
  ...
  const kickoffCard=el("section",{class:"tactics-match-card","aria-label":`${opponent.name} 대 대한민국 경기 준비`},[
    ...
    el("div",{class:"tactics-match-card__teams"},[
      el("div",{class:"tactics-match-card__team"},[CountryFlag({teamId:opponent.id,size:"medium"}),el("strong",{text:opponent.name})]),
  ...
  function buildMatchSetup() {
    if (!opponent)
      return { matchSetup: null, note: "상대가 아직 결정되지 않았습니다." };
    ...
    if (!check.ok)
      return {
        matchSetup: null,
        note: check.errors.map((e) => e.message).join(" / "),
      };
  ...
  function updateKickoff() {
    ...
    floatingKickoffBtn.disabled = !ok;
    kickoffBtn.title = ok
      ? ""
      : "선발 11명이 유효해야 경기를 시작할 수 있습니다.";
    if (!ok) {
  ...
  document.querySelector(".top-navigation")?.append(floatingKickoffBtn);
  const hero=root.querySelector(".topbar");
  const updateFloatingKickoff=()=>{
    const navigationHeight=parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--app-navigation-height"))||62;
    floatingKickoffBtn.hidden=hero.getBoundingClientRect().bottom>navigationHeight;
  };
  window.addEventListener("scroll",updateFloatingKickoff,{passive:true});
  window.addEventListener("resize",updateFloatingKickoff);
  updateFloatingKickoff();
  ...
  return () => {
    clearTimeout(saveTimer);
    window.removeEventListener("scroll",updateFloatingKickoff);
    window.removeEventListener("resize",updateFloatingKickoff);
    floatingKickoffBtn.remove();
    persist();
    editor.destroy();
  };
}
