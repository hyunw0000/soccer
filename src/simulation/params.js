...
export const PARAMS = {
  ...
  // 드리블 vs 패스 균형 — 실전에서 드리블이 압도적으로 이겨서(90분 한 판 판단 96회 중 64회)
  // 패스가 거의 안 나오고 볼이 옆/넓게 못 퍼졌다. 드리블 총점을 한 단계 낮추고(scale) 주변
  // 압박에는 더 민감하게(pressureCoef↑) 만들어 패스와 정직하게 경쟁시킨다.
  dribbleScoreScale: 0.55,
  dribblePressureCoef: 0.3,

  // 폭 지시가 없어도(기본값 0.5) 측면 옵션에 최소한의 가산을 준다 — 없으면 forwardGain이
  // 항상 이겨서 지시 없는 팀은 폭을 아예 안 쓴다(실전 확인). "좁게"(0) 지시는 여전히 이 값을
  // 상쇄하고도 남을 만큼 감산이 커서(§6.5 widthBias 공식 참고) 명시적 지시는 그대로 먹힌다.
  widthDefaultBias: 0.35,
};
